...
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.LICENSE_AGREENMENT_WEBSITE;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.PRIVACY_POLICY_WEBSITE;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.PUBLISHER_EMAIL;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.PUBLISHER_WEBSITE;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_FILE_NAME_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_PACK_ICON_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_PACK_NAME_IN_QUERY;
import static org.bhujmandir.swaminarayan.stickers.StickerContentProvider.STICKER_PACK_PUBLISHER_IN_QUERY;

public class StickerPackLoader {

    /**
     * Get the list of stickers packs for the stickers content provider
     */
    @NonNull
    public static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        ...
        for (StickerPack stickerPack : stickerPackList) {
            if (identifierSet.contains(stickerPack.identifier)) {
                throw new IllegalStateException("stickers pack identifiers should be unique, there are more than one pack with identifier:" + stickerPack.identifier);
            } else {
                identifierSet.add(stickerPack.identifier);
            }
        }
        if (stickerPackList.isEmpty()) {
            throw new IllegalStateException("There should be at least one stickers pack in the app");
        }
        for (StickerPack stickerPack : stickerPackList) {
    ...
    private static List<Sticker> getStickersForPack(Context context, StickerPack stickerPack) {
        ...
        for (Sticker sticker : stickers) {
            ...
            try {
                ...
                if (bytes.length <= 0) {
                    throw new IllegalStateException("Asset file is empty, pack: " + stickerPack.name + ", stickers: " + sticker.imageFileName);
                }
                sticker.setSize(bytes.length);
            } catch (IOException | IllegalArgumentException e) {
                throw new IllegalStateException("Asset file doesn't exist. pack: " + stickerPack.name + ", stickers: " + sticker.imageFileName, e);
            }
        }
        return stickers;
    }
    ...
    public static byte[] fetchStickerAsset(@NonNull final String identifier, @NonNull final String name, ContentResolver contentResolver) throws IOException {
        try (final InputStream inputStream = contentResolver.openInputStream(getStickerAssetUri(identifier, name));
            ...
            if (inputStream == null) {
                throw new IOException("cannot read stickers asset:" + identifier + "/" + name);
            }
            int read;
