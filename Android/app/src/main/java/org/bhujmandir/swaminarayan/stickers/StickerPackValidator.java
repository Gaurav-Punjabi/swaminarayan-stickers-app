...
public class StickerPackValidator {
    ...
    /**
     * Checks whether a stickers pack contains valid data
     */
    static void verifyStickerPackValidity(@NonNull Context context, @NonNull StickerPack stickerPack) throws IllegalStateException {
        if (TextUtils.isEmpty(stickerPack.identifier)) {
            throw new IllegalStateException("stickers pack identifier is empty");
        }
        if (stickerPack.identifier.length() > CHAR_COUNT_MAX) {
            throw new IllegalStateException("stickers pack identifier cannot exceed " + CHAR_COUNT_MAX + " characters");
        }
        checkStringValidity(stickerPack.identifier);
        if (TextUtils.isEmpty(stickerPack.publisher)) {
            throw new IllegalStateException("stickers pack publisher is empty, stickers pack identifier:" + stickerPack.identifier);
        }
        if (stickerPack.publisher.length() > CHAR_COUNT_MAX) {
            throw new IllegalStateException("stickers pack publisher cannot exceed " + CHAR_COUNT_MAX + " characters, stickers pack identifier:" + stickerPack.identifier);
        }
        if (TextUtils.isEmpty(stickerPack.name)) {
            throw new IllegalStateException("stickers pack name is empty, stickers pack identifier:" + stickerPack.identifier);
        }
        if (stickerPack.name.length() > CHAR_COUNT_MAX) {
            throw new IllegalStateException("stickers pack name cannot exceed " + CHAR_COUNT_MAX + " characters, stickers pack identifier:" + stickerPack.identifier);
        }
        if (TextUtils.isEmpty(stickerPack.trayImageFile)) {
            throw new IllegalStateException("stickers pack tray id is empty, stickers pack identifier:" + stickerPack.identifier);
        }
        ...
        if (stickers.size() < STICKER_SIZE_MIN || stickers.size() > STICKER_SIZE_MAX) {
            throw new IllegalStateException("stickers pack stickers count should be between 3 to 30 inclusive, it currently has " + stickers.size() + ", stickers pack identifier:" + stickerPack.identifier);
        }
    ...
    private static void validateSticker(@NonNull Context context, @NonNull final String identifier, @NonNull final Sticker sticker) throws IllegalStateException {
        if (sticker.emojis.size() > EMOJI_LIMIT) {
            throw new IllegalStateException("emoji count exceed limit, stickers pack identifier:" + identifier + ", filename:" + sticker.imageFileName);
        }
    ...
    private static void validateStickerFile(@NonNull Context context, @NonNull String identifier, @NonNull final String fileName) throws IllegalStateException {
        try {
            ...
            if (bytes.length > STICKER_FILE_SIZE_LIMIT_KB * ONE_KIBIBYTE) {
                throw new IllegalStateException("stickers should be less than " + STICKER_FILE_SIZE_LIMIT_KB + "KB, stickers pack identifier:" + identifier + ", filename:" + fileName);
            }
            try {
                final WebPImage webPImage = WebPImage.create(bytes);
                if (webPImage.getHeight() != IMAGE_HEIGHT) {
                    throw new IllegalStateException("stickers height should be " + IMAGE_HEIGHT + ", stickers pack identifier:" + identifier + ", filename:" + fileName);
                }
                if (webPImage.getWidth() != IMAGE_WIDTH) {
                    throw new IllegalStateException("stickers width should be " + IMAGE_WIDTH + ", stickers pack identifier:" + identifier + ", filename:" + fileName);
                }
                if (webPImage.getFrameCount() > 1) {
                    throw new IllegalStateException("stickers shoud be a static image, no animated stickers support at the moment, stickers pack identifier:" + identifier + ", filename:" + fileName);
                }
            } catch (IllegalArgumentException e) {
                throw new IllegalStateException("Error parsing webp image, stickers pack identifier:" + identifier + ", filename:" + fileName, e);
            }
        } catch (IOException e) {
            throw new IllegalStateException("cannot open stickers file: stickers pack identifier:" + identifier + ", filename:" + fileName, e);
        }
    }
